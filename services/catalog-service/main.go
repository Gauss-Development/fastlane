...
import (
	...
	grpcinterface "catalog-service/internal/interfaces/grpc"
	"catalog-service/pkg/logger"
	"catalog-service/pkg/metrics"
...
func main() {
	...
	mfrRepo := postgres.NewManufacturerRepository(db)
	mfrSvc := services.NewManufacturerService(mfrRepo, appLogger)
	...
	manufacturerv1.RegisterManufacturerServiceServer(grpcServer, grpcinterface.NewManufacturerServer(mfrSvc, appLogger))
	...
	go func() {
		appLogger.Info("Catalog gRPC service starting on port " + cfg.GRPCPort)
	...
	router.Use(metrics.GinMiddleware("catalog-service"))
	...
	go func() {
		appLogger.Info("Catalog service starting on port " + cfg.Port)
