...
export function toSuccessResponse<T>(
  ...
  if (setCookie) {
    response.headers.set("set-cookie", devSafeCookie(setCookie));
  }

  return response;
}

// The gateway runs with ENVIRONMENT=production in Docker and therefore marks the
// refresh-token cookie `Secure`. In local dev the browser talks to the Next dev
// server over http://localhost, where Secure cookies are silently dropped by
// some browsers (notably Safari) — so the session never persists. Strip `Secure`
// only in development; production (served over HTTPS) keeps it.
function devSafeCookie(cookie: string): string {
